...
import { getClashProxyConfig, getProviderList, getShadowrocketProxyConfig } from '@server/services/provider'
import { Env } from '@server/types/data'
...
router.get('/list', auth, async (ctx) => {
  ...
  const providers = await getProviderList(user.uid)
  return ctx.json({
    providers: providers,
  })
})
